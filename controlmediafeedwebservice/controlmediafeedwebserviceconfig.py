##
#        File: controlmediafeedwebserviceconfig.py
#     Created: 11/18/2020
#     Updated: 11/18/2020
#  Programmer: Cuates
#  Updated By: Cuates
#     Purpose: Control media feed web service configuration
##
...
# Class
class ControlMediaFeedWebServiceConfig:
  ...
  def _setConfigVars(self, type):
    ...
    if not regEx.search(r'\b' + "|".join(ServerType) + r'\b', ServerInfo, flags=regEx.IGNORECASE):
      ...
      if type == 'MariaDBSQLControlMedia':
        # Set variables
      ...
      elif type == 'MSSQLWControlMedia':
        # Set variables
        self._driver = 'ODBC Driver 17 for SQL Server'
        self._databasedialect = 'mssql+pyodbc:///'
        self._servername = '192.168.1.2'
        self._port = ''
        self._database = ''
        self._username = ''
        self._password = ''
  ...
  def _setFilenameVars(self, feedAction):
    ...
    if regEx.search(r'\bLog\b', feedAction, flags=regEx.IGNORECASE):
      self._pathParent = '/path/to/controlmediafeedwebservice'
      self._pathLevelOne = '/resource'
      self._pathLevelTwo = '/log'
      self._jsonConfigLogFilename = '/logging_dictConfig.json'
      self._filenameMedia = '/controlmediafeedwebservicepy.log'
